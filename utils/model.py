...
### Code adapted from https://github.com/lucidrains/CoCa-pytorch/blob/main/coca_pytorch/coca_pytorch.py
### We added some new functions to get the model outputs and accuracy, and make the model generate autoregressively in a loop for testing
...
class Residual(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        func_output = self.fn(x, *args, **kwargs)

        # NEW CODE - return attn if self.fn returns attn
        if isinstance(func_output, tuple):
            out, attn = func_output

            return out + x, attn
        
        return func_output + x
...
class CoCa(nn.Module):
    ...
    def embed_image(self, images=None, image_tokens=None):
        ...
        img_queries, attn  = self.img_attn_pool(img_queries, image_tokens) # NEW CODE - return attention for visualization
        img_queries = self.img_attn_pool_norm(img_queries)

        return img_queries[:, 0], img_queries[:, 1:], attn # NEW CODE - return attention for visualization
    ...
    def forward(
        ...
        image_embeds, image_tokens, token_attn = self.embed_image(images=images, image_tokens=image_tokens)
        ...
        for attn_ff, cross_attn in self.multimodal_layers:
            ...
            # NEW CODE - return attention for visualization
            text_tokens, attn = cross_attn(text_tokens, image_tokens) 
            if count == layer:  # Save attention weights only for specified layer
                query_attn = attn
            count+=1
        ...
        ## NEW CODE
        predicted_labels = logits.argmax(dim=1).cpu().numpy() # Shape: (batch_size, num_tokens)
        outputs = self.tokens_to_smiles(predicted_labels, labels, tokenizer)
        accuracy = self.get_accuracy(outputs)

        if return_attention:
            return caption_loss + contrastive_loss, accuracy, outputs, query_attn, token_attn # NEW CODE - return attention for visualization
        
        return caption_loss + contrastive_loss, accuracy, outputs
    
    ### NEW CODE
    def tokens_to_smiles(self, predicted_labels, labels, tokenizer):
        """
        Get the predicted and original SMILES strings from the model outputs
        """
        outputs = []
        for i, predicted_tokens in enumerate(predicted_labels):
            if tokenizer.sep_token_id in predicted_tokens:
                sep_token_index = predicted_tokens.tolist().index(tokenizer.sep_token_id)
                predicted_tokens = predicted_tokens[:sep_token_index]
            outputs.append({
                    'original': tokenizer.decode(labels[i].tolist()),
                    'predicted': tokenizer.decode(predicted_tokens.tolist()),
            })
        return outputs
    ...
    def get_accuracy(self, outputs):
        ...
        for output in outputs:
            if output['original'] == output['predicted']:
                num_correct += 1
        return num_correct / len(outputs)
    
    ### NEW CODE
    def generate_autoregressively(self, tokenizer, spectrum, smiles, max_length=23, num_attempts=1):
        """
        Generate SMILES strings autoregressively, with multiple attempts for each spectrum
        """
        batch_size = spectrum.shape[0]
        attempts = []
        for i in range(num_attempts):
            # Prepare a batch of start tokens
            start_token = torch.tensor([tokenizer.cls_token_id], dtype=torch.long, device=spectrum.device)
            generated_toks = start_token.repeat(batch_size, 1)
            
            for _ in range(max_length):
                logits = self.forward(tokenizer, generated_toks, spectrum, return_loss=False)
                logits = torch.nn.functional.softmax(logits[:,-1], dim=-1)
                if i == 0:
                    next_token = torch.argmax(logits, dim=-1, keepdim=True) # Just grab the most likely token
                else:
                    next_token = torch.multinomial(logits, num_samples=1) # Sample the next token with probabilities
                generated_toks = torch.cat([generated_toks, next_token], dim=1)
            attempts.append(generated_toks)

        # Convert tokens to SMILES strings
        attempts = [self.tokens_to_smiles(attempt, smiles, tokenizer) for attempt in attempts]

        # Combine the attempts into a single list of outputs
        final = [{'original': None, 'predicted': []} for _ in range(batch_size)]
        for attempt in attempts:
            for i, output in enumerate(attempt):
                final[i]['predicted'].append(output['predicted'])
                final[i]['original'] = output['original']
        return final